            ...
            <div class="tabbable">
                ...
                <div class="tab-content">
                    <div class="tab-pane fade in active">
                        <fieldset>
                            <legend>第二种storage</legend>
                            <form class="form-horizontal" action="{{ Route('storageUpload') }}" method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                        ...
                        <table class="table table-condensed table-hover" id="sample-table-4">
                            ...
                            @foreach($datas as $data)
                                <tr>
                                    <td>{{ $data->id }}</td>
                                    <td class="hidden-xs"><a target="_blank" href="/blog/admin/Function-Manage/Upload-File/storageUploadtest/{{ $data->filename }}">{{ $data->filename }}</a></td>
                                    <td>{{ $data->created_at }}</td>
                                    <td class="hidden-xs"><span class="label label-sm label-success">第二种上传</span></td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
